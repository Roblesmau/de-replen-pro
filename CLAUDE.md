...
| Capacity total on screen disagrees with re-exported Excel (e.g. footer says 28,921 but Excel exports 29,000+) | Footer total only counts cells passing `STORE_BRANDS[sid].has(brand)` AND brand+type in `catalogCombos` — orphaned combos (brand typos, casing diffs, types not in live Shopify catalog) get hidden but stay in `S.CAP` and survive export | Footer now shows BOTH `visible total` (filtered scope) and `stored grand total` (everything in `S.CAP` — matches Excel). When they differ, an amber pill shows hidden units + count of orphaned combos with a `show` button (`showOrphanedCapacity`) listing the offending pairs and likely root cause (Excel typo vs catalog not loaded yet) |
