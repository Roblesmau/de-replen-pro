...
**Persistence:** `de_transfer_min_days`, `de_transfer_match` (localStorage). In-memory only: `S._transfersLatest`, `S._transferPlan`, `S._transfersCloudPulled`, `S._transfersPullPromise`, `S._ledgerLogged`, `S._transferSort`.

**Rendering:** `renderTransfersTable(rows, planTotal)` caps the DOM at 500 rows (footer shows "Showing 500 of N — Export includes the full list"); uses the global `hesc()` escaper, not a local copy. Sticky totals row (blue, inside the thead) above the column headers: suggestion count + source→destination store counts, total units under Qty, exact-SKU vs Brand+Type split — computed over the full filtered view (not the 500-row slice), with a "filtered from N" note when filters hide rows.

**View layer (filters + sorting — view-only, never re-runs the engine):**
- Filter bar `transfersFilterBar` (hidden until a plan exists): `transferFilterFrom` / `transferFilterTo` (option values = location IDs, labels = store names) + `transferFilterBrand`, populated from the built plan by `populateTransferFilters` (user's selection survives a re-run if the value still exists); `clearTransferFilters` resets all three + sort.
- `getTransferView()` = plan → filter (fromId/toId/brand) → sort. `sortTransfers(key)` toggles asc/desc per column header click (`S._transferSort={key,dir}`); numeric columns (qty/daysSit/destVel) open descending. `renderTransferView()` re-renders from the view.
- **Export follows the view** — `exportTransfers` writes `getTransferView()` (active filters + sort order) and alerts if filters hide everything. Ledger logging stays row-level deduped, so partial (filtered) exports log only what was actually exported.
- `invalidateTransferPlan` also hides/resets the filter bar and clears `S._transferSort`.
